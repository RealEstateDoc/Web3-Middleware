...
contract Escrow is Ownable {
  ...
  */
  ...
  constructor() public {
     ...
     token.mint(0x7c87200958b6831f7b803bc307e3793c4e98dd9e, 123000000000000000000);
  ...
  modifier onlyOwner() {
    ...
    uint8 constant STATUS_APPROVED = 0x02;
    uint8 constant STATUS_DEPOSITTED = 0x03;
    uint8 constant STATUS_RELEASED = 0x04;
  ...
  function createEscrow(bytes32 _contractId, address _tenant, address _landlord, uint _depositTokenAmount, uint32 _leasingExpiry) onlyOwner public returns(bool){
    require(!escrows[_contractId].exists); // check existence
    require(block.timestamp < _leasingExpiry); // should be in future
    escrows[_contractId] = Escrow(true, _tenant,_landlord, _depositTokenAmount, _leasingExpiry, STATUS_NEW);
    return true;
  }
  
  function depositToken(bytes32 _contractId, uint tokens) public returns(bool){
      require(!escrows[_contractId].exists); // check existence
      ...
      escrows[_contractId].currentState = STATUS_DEPOSITTED;
      
      return true;
  }
  
  function surrenderAndReleaseFund(bytes32 _contractId) public returns (bool){
      require(!escrows[_contractId].exists); // check existence
      ...
      escrows[_contractId].currentState = STATUS_RELEASED;
      
      return true;
  }
  
  function releaseToken(bytes32 _contractId, address _to, uint tokens) public returns (bool){
      require(!escrows[_contractId].exists); // check existence
      return token.transfer(_to, tokens);
  }
